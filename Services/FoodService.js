...
class FoodService{
    static async getFoodInfo(id){
        let response = await (new SQLBuilder())
            ...
            .request();
        response = Adapter.deleteCols(response, ["restaurants_id_restaurants", "price"]);
    ...
    static async getFoodList(restId){
        ...
        data = Adapter.renameCols(data, {"id": "id_food_items", "image": "food_image_href"});
        return data;

    }
    static async addFoodItem(data){
        const resp = await (new SQLBuilder())
            .insert(FoodItems.tableName, FoodItems.colNames, ["id_food_items", "food_image_href"])
